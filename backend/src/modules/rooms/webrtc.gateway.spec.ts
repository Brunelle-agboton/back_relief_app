...
import { TokenType } from '../auth/jwt.constants';

// ─── Helpers ────────────────────────────────────────────────────────────────

function makeSocket(
  overrides: Partial<Socket> & { user?: any } = {},
): jest.Mocked<Socket> & { user?: any } {
...
describe('WebrtcGateway', () => {
  ...
  // SEC-08 : le gateway n'accepte plus qu'un access token, et exige JWT_SECRET
  // (le repli `|| 'secretKey'` a été supprimé).
  const validPayload = {
    sub: 1,
    email: 'user@test.com',
    role: 'user',
  ...
  describe('handleConnection', () => {
    it('accepte une connexion avec un token valide', async () => {
      ...
      expect((client as any).user).toEqual({
        userId: 1,
        email: 'user@test.com',
        role: 'user',
      });
    ...
    it('refuse un refresh token présenté sur le canal temps réel', async () => {
